...
// ─── TOP 100 Tickers — полностью динамически from MOEX ─────────────────────
// Хардкорный список УДАЛЁН — используем только MOEX API
...
export interface TickerScanResult {
  ...
  cumDelta: number;
  ofi: number;
  realtimeOFI?: number;   // Real-time OFI (OB или trade-based)
  ofiSource?: 'orderbook' | 'trades';  // Sprint 5B: источник OFI
  turnover: number;
...
export async function scanTicker(
  ...
  try {
    ...
    return {
      ...
      ofiSource: detectorInput.ofiSource,
      turnover,
      moexTurnover,
      type: tickerType,
      taContext,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // НО: если сессия MAIN/EVENING и есть тикеры для сканирования → это утренний запуск, не закрытие
    ...
    // Market closed ТОЛЬКО если: нет данных AND (нет тикеров OR не торговая сессия)
