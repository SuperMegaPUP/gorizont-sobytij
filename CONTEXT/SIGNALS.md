...
```
Пример: BSCI 0.63, базовая конвергенция 8/10
  −2 СПУФИНГ → 6/10 → НИЖЕ ПОРОГА 7 → сигнал НЕ генерируется ✅

Пример без спуфинга: BSCI 0.63, базовая конвергенция 8/10
  +1 роботы → 9/10 → ВЫШЕ ПОРОГА 7 → сигнал генерируется ✅
```
...
7. **Feedback loop** — каждый сигнал получает result + pnl для калибровки
8. **Спуфинг-фильтр** — штрафы конвергенции автоматически блокируют ненадёжные сигналы
