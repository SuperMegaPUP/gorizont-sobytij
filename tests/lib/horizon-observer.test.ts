...
describe('Observer Pipeline: detectors + BSCI on synthetic data', () => {
  test('runAllDetectors returns 17 results on valid input', async () => {
  ...
  test('calcBSCI on detector results produces valid composite', async () => {
    ...
    const detectorNames = ['GRAVITON', 'DARKMATTER', 'ACCRETOR', 'DECOHERENCE', 'HAWKING', 'PREDATOR', 'CIPHER', 'ENTANGLE', 'WAVEFUNCTION', 'ATTRACTOR', 'ROTATION', 'ALGORITHM', 'SQUEEZE', 'PREIMPULSE', 'ICEBERG', 'DISTRIBUTION', 'PHASE_SHIFT'];
    ...
    expect(bsci.scores).toHaveLength(17);
  });
